...
功能：
- 读取 `config/blog-index.yaml` 中的文章信息
- 按日期降序排序文章
- 生成符合RSS 2.0规范的XML文件
- 自动更新 `rss.xml` 文件
...
2. 在 `config/blog-index.yaml` 中添加文章信息：
   ```yaml
   articles:
     - title: "文章标题"
       date: "YYYY-MM-DD"
   ...
   ```
3. 运行 `scripts/generate_md_pages.py` 生成HTML页面
4. 运行 `scripts/generate_rss.py` 更新RSS订阅源

## 工具页面管理

1. 在 `pages` 目录下创建工具页面
2. 在 `config/tools-index.yaml` 中添加工具信息：
   ```yaml
   tools:
     - title: "工具名称"
       description: "工具描述"
...
2. 在 `config/resources-index.yaml` 中添加资源信息：
   ```yaml
   resources:
     - title: "资源名称"
       description: "资源描述"
...
1. 在 `config/favorites.yaml` 中添加收藏信息：
   ```yaml
   favorites:
     分类名称:
       - title: "收藏标题"
